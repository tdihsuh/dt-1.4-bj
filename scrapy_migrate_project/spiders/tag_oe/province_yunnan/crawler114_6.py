...
class Crawler1146Spider(scrapy.Spider):
    name = 'crawler114_6'
    ...
    def start_requests(self):
        url_page = 'http://yn.gsxt.gov.cn/notice/search/GET/announce?type=0101&mode=all&pageNo=1&areaId=&keyword='
        r = requests.get(url_page, headers=self.headers)
        json_content = r.json()
        totalpages = json_content["result"]["pageCount"]

        for i in range(1, totalpages):
            url = 'http://yn.gsxt.gov.cn/notice/search/GET/announce?type=0101&mode=all&pageNo=' + str(
                i) + '&areaId=&keyword='
