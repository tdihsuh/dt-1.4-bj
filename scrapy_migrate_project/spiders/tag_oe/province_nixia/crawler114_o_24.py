# -*- coding: utf-8 -*-
# 国家企业信用信息系统(宁夏) 企业经营异常名录-移出
import scrapy
import time
from scrapy_migrate_project.items import crawler114_out


class Nx024OutSpider(scrapy.Spider):
    name = 'crawler114_o_24'
    allowed_domains = ['nx.gsxt.gov.cn']
    start_urls = ['http://nx.gsxt.gov.cn/noticeAction_noticeList.action?noticeType=12&currPage=1']
    ...
    def parse(self, response):
        ...
        for li in news_li:
            ...
            item['release_org'] = li.xpath('./span/text()').extract_first()
        ...
        if int(cur_page) < int(total_pages):
            ...
            next_href = 'http://nx.gsxt.gov.cn/noticeAction_noticeList.action?noticeType=12&currPage='+ str(cur_page)
