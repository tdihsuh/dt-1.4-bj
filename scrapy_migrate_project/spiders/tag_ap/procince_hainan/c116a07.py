...
import re
import time
class C116a07Spider(scrapy.Spider):
    name = 'c116a07'
    allowed_domains = ['*']
    ...
    def parsePageDetail(self,response):
        ...
        if not self.patt.search(response.text):
            ...
            if not soup.find('div',class_=re.compile(r'ip')):
                ...
                item['law_item'] = ''
                item['credit_no'] =soup.find_all('tr')[2].find_all('td')[1].get_text(strip=True).replace(' ','').replace('\t','').replace('\n','').replace('\r','')
                item['reg_no'] = soup.find_all('tr')[2].find_all('td')[-1].get_text(strip=True).replace(' ','').replace('\t','').replace('\n','').replace('\r','')
                item['tax_no'] = ''
                item['identity_card'] =''
                item['legal_man'] =soup.find_all('tr')[6].find_all('td')[-1].get_text(strip=True).replace(' ','').replace('\t','').replace('\n','').replace('\r','')

                item['punish_result']=soup.find_all('tr')[9].find_all('td')[-1].get_text(strip=True).replace(' ','').replace('\t','').replace('\n','').replace('\r','')
                item['current_status'] = ''
                item['area_code'] = ''
                item['offical_updtime'] =soup.find_all('tr')[10].find_all('td')[-1].get_text(strip=True).replace(' ','').replace('\t','').replace('\n','').replace('\r','')
